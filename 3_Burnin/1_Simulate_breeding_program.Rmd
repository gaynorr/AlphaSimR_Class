---
title: "Simulating a breeding program"
output: html_notebook
---

This notebook will briefly outline a strategy for writing breeding program simulations in AlphaSimR using a very simple clonal breeding program as an example. This example will model 20 years of this breeding program. The example will also generate plots show the change in mean genetic value and genetic variance over time for the first stage of the breeding program.

Complete code for the simulation is provided at the end. The steps in-between demonstrate the process used to write this code. Note that some of intermediate steps provide invalid code. You shouldn't run code for the intermediate steps. Only run the final, complete code.
...
Make 100 bi-parental crosses at random using all clones from Stage 2 and Stage 3 testing as parents. The total number of parents will be 60.
...
Select the best 50 clones from Stage 1 testing for Stage 2 testing. Stage 2 testing is performed using four replicate plots and has a residual error variance of 4.

#### Year 5 (Stage 3)

Select the best 10 clones from Stage 2 testing for Stage 3 testing. Stage 3 testing is performed using eight replicate plots and has a residual error variance of 4.
...
The purpose of this stage is to place unique individuals in every stage of the breeding program. We don't want the exact same individuals to be in multiple stages of the breeding program at the same time. We will generate all individuals from the initial parents, so they won't have generational difference we'd expect in a real breeding program. This generational difference will be created later simulating future years of breeding.
...
Then, the code for reporting the results then needs to occur inside the loop for simulating 20 years of breeding.
